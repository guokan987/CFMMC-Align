...
def process_questions_oie_keybert_bert2(args):
    ...
    with StanfordOpenIE(properties=properties) as client:
        if args.mode in ['train']:
            ...
            for instance in instances:
                data: list = json.loads(instance.strip())
                options: list = data[5:9]#4个选项的文本
                answer_idx: int = data[9]#正确答案选项
...
def process_questions_oie_keybert_bert3(args):
    ...
    model,_ = clip.load("ViT-B-32.pt", device='cuda:0')
    ...
    with StanfordOpenIE(properties=properties) as client:
        if args.mode in ['train']:
            ...
            for i, instance in enumerate(instances):
                data: list = json.loads(instance.strip())
                answer1: str = data[5].lower()
                answer2: str = data[6].lower()
                answer3: str = data[7].lower()
                answer4: str = data[8].lower()
                question: str = data[4].lower()[:-1]
        ...
        for idx, instance in enumerate(instances):
            ...
            q_ids: int = data[0]
            question: str = data[4].lower()[:-1]
            candidates1: str = data[5].lower()
            candidates2: str = data[6].lower()
            candidates3: str = data[7].lower()
            candidates4: str = data[8].lower()
            ...
            question_encoded = utils.encode(question_tokens, vocab['question_token_to_idx'], allow_unk=True)
            questions_encoded.append(question_encoded)
            questions_len.append(len(question_tokens))
            while len(question_tokens)<32:
                question_tokens.append('NULL')
            questions_clip.append(clip.tokenize(question).numpy())
            question_ids.append(q_ids)
            
            
            answer_idx1: int = data[9]
            answer: str = candidates[answer_idx1]#正确答案文本
            
            answer_tokens1 = nltk.word_tokenize(answer)
            answer_len.append(len(answer_tokens1))
            while len(answer_tokens1)<32:
                answer_tokens1.append('NULL')
            answer_encoded.append(clip.tokenize(answer).numpy())
            answer_ids.append(q_ids)
            ...
            for ans in candidates:
                ans = ans.lower()
                ans_tokens = nltk.word_tokenize(ans)
                candidates_len.append(len(ans_tokens))
                while len(ans_tokens)<32:
                    ans_tokens.append('NULL')
                candidates_encoded.append(clip.tokenize(ans).numpy()) 
            ...
            if (idx % 5000 == 0):
                print('{:d}/{:d} {:.3f}s (projected finish: {:.2f} hours)' \
                      .format(i1, count, _t['misc'].average_time,
                              _t['misc'].average_time * (count - i1) / 3600))
        # Pad encoded answer candidates
        for qe in questions_encoded:
            while len(qe) < 32:
                qe.append(vocab['question_token_to_idx']['<NULL>'])
        ...
        questions_encoded = np.asarray(questions_encoded, dtype=np.int32)
        questions_clip = np.asarray(questions_clip, dtype=np.int32)
        questions_len = np.asarray(questions_len, dtype=np.int32)
        print(questions_encoded.shape)
