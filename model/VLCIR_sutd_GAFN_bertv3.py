...
class InputUnitVisual_GST_Transformer_GAFN(nn.Module):
    def __init__(self, motion_dim, appearance_dim, module_dim=512):#motion_feature,frame_feature
        ...
        self.clip_level_motion_proj = nn.Sequential(
                                                ...
                                                nn.GELU(),
                                                nn.Dropout(p=0.1),                                             
                                                    )#全连接：将clip特征维度映射到统一的特征维度

        self.appearance_feat_proj = nn.Sequential(
                                                nn.Linear(appearance_dim, module_dim),
                                                nn.GELU(),
                                                nn.Dropout(p=0.1),                                             
                                                    )#全连接：将frame特征维度映射到统一的特征维度                                        
        self.clip_level_motion_VLAD_proj = nn.Sequential(
        ...
        self.QATransformer =  TransformerEncoder(embed_dim=module_dim, pos_flag='learned',pos_dropout=0.1,num_heads=8,attn_dropout=0.1,res_dropout=0.1,activ_dropout=0.1,activation='gelu',num_layers=3)#Q+frame
        self.QMTransformer =  TransformerEncoder(embed_dim=module_dim, pos_flag='learned',pos_dropout=0.1,num_heads=8,attn_dropout=0.1,res_dropout=0.1,activ_dropout=0.1,activation='gelu',num_layers=3)#ques+clip
        self.VIATransformer =  TransformerEncoder(embed_dim=module_dim, pos_flag='learned',pos_dropout=0.1,num_heads=8,attn_dropout=0.1,res_dropout=0.1,activ_dropout=0.1,activation='gelu',num_layers=3)#mm+frame
        self.VIMTransformer =  TransformerEncoder(embed_dim=module_dim, pos_flag='learned',pos_dropout=0.1,num_heads=8,attn_dropout=0.1,res_dropout=0.1,activ_dropout=0.1,activation='gelu',num_layers=3)#mm+clip
        self.module_dim = module_dim
    ...
    def forward(self, appearance_video_feat, motion_video_feat, question_embedding,question_embedding_do, appearance_dict, motion_dict):
        ...
        Args:
            ...
            motion_video_feat: [Tensor] (batch_size, num_clips, visual_inp_dim)
            question_embedding: [Tensor] (batch_size, module_dim)
            appearance_dict：the cluster feature of appearance_video_feat
        return:
            encoded video feature: [Tensor] (batch_size, N, module_dim)
        """
        visual_embedding_motion = self.clip_level_motion_proj(motion_video_feat)#.permute(1,0,2)
        ...
        visual_embedding_motion_vlad2 = self.net_vlad_motion2(visual_embedding_motion,visual_embedding_motion)#自己做软注意力
        ...
        #上述代码得到一个层次视觉特征
        question_visual_m = self.QMTransformer(question_embedding,visual_embedding_motion.permute(1,0,2))#Transformer 融合多模态特征
        question_visual_m_do = self.QMTransformer(question_embedding_do,visual_embedding_motion_vlad.permute(1,0,2))#Transformer 融合多模态特征
        question_visual_a  = self.QATransformer(question_embedding,torch.mean(visual_embedding_appearance.permute(1,2,0,3), 1))
...
class VLCIR(nn.Module):#主模型
    def __init__(self, motion_dim, appearance_dim, module_dim, word_dim,  vocab, vocab_subject, vocab_relation, vocab_object, question_type):
        ...
        self.feature_aggregation = FeatureAggregation_four_st(module_dim)#特征聚合1
        self.feature_aggregation2 = FeatureAggregation(module_dim)#特征聚合2
        self.feature_aggregation3 = FeatureAggregation_four_st2(module_dim)#特征聚合3
        ...
        if self.question_type in ['action']:
            ...
            self.linguistic_input_unit_do = InputUnitLinguistic_Transformer(vocab_size=encoder_vocab_size, wordvec_dim=word_dim,module_dim=module_dim)#do去除统计概率的影响，专注输入特征
            ...
            self.linguistic_subject_input_unit = InputUnitLinguistic_subject_Bert(vocab_size=encoder_vocab_subject_size, wordvec_dim=word_dim,module_dim=module_dim)#bert来编码
            ...
            self.fusion_do=SE_Fusion_Four(4*module_dim,4*module_dim,4*module_dim,4*module_dim,4).cuda()#融合多个特征去处统计因素
    ...
    def forward(self, ans_candidates, ans_candidates_len, video_appearance_feat, video_motion_feat,\
                    ...
                    question, question_subject, question_relation, question_object, question_len, question_subject_len, question_relation_len, question_object_len, appearance_dict, motion_dict,ques_type):
        """
        Args:
        ...
        if self.question_type in ['action']:
        ...
        elif self.question_type in ['frameqa', 'none']:#sutd的模块处
            # get image, word, and sentence embeddings
            question_embedding_bert = self.linguistic_input_unit(self.BertEncoder,question, question_input_bert,question_mask_bert,question_ids_bert, question_bert_len)#bert编码的问题文本特征
            question_embedding_bert_do = self.linguistic_input_unit_do(self.BertEncoder,question, question_input_bert,question_mask_bert,question_ids_bert,question_bert_len)#消除数据bias的模型，bert编码的问题文本特征
            question_embedding_subject_bert = self.linguistic_subject_input_unit(self.BertEncoder,question_subject, subject_input_bert,subject_mask_bert,subject_ids_bert, question_subject_bert_len)#bert编码的问题主语文本特征
            ...
            question_embedding_relation_bert = self.linguistic_relation_input_unit(self.BertEncoder,question_relation, relation_input_bert,relation_mask_bert,relation_ids_bert, question_relation_bert_len)##bert编码的问题关系（谓语）文本特征
            ...
            question_embedding_object_bert = self.linguistic_object_input_unit(self.BertEncoder,question_object, object_input_bert,object_mask_bert,object_ids_bert, question_object_bert_len) ##bert编码的问题目标（宾语）文本特征
            ...
            question_embedding, question_embedding_do, visual_embedding, visual_embedding_do \
                = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding_bert, question_embedding_bert_do, appearance_dict, motion_dict )#问题视觉语义多模态特征编码
            question_embedding_subject, question_embedding_subject_do, visual_embedding_subject, visual_embedding_do_subject \
                = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding_subject_bert, question_embedding_subject_bert_do, appearance_dict, motion_dict )#问题主语视觉语义多模态特征编码
            question_embedding_relation, question_embedding_relation_do, visual_embedding_relation, visual_embedding_do_relation \
                = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding_relation_bert, question_embedding_relation_bert_do, appearance_dict, motion_dict )#问题谓语视觉语义多模态特征编码
            question_embedding_object, question_embedding_object_do, visual_embedding_object, visual_embedding_do_object \
                = self.visual_input_unit(video_appearance_feat, video_motion_feat, question_embedding_object_bert, question_embedding_object_bert_do, appearance_dict, motion_dict )#问题宾语视觉语义多模态特征编码
            ...
            out = self.output_unit(torch.cat((question_embedding_all,question_embedding_all_do),1),torch.cat((visual_embedding_all,visual_embedding_all_do),1))#输出特征
