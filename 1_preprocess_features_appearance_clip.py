    ...
    with h5py.File(outfile, 'w') as fd:
        ...
        for i, (video_path, video_id) in enumerate(video_ids):
            ...
            if args.feature_type == 'appearance':
                ...
                if valid:
                    for clip_id, clip in enumerate(clips):
                        if args.dataset=='sutd-qa':
                            feats = run_batch_sutd(clip, model)  # (8, 768)
                        else:
                            feats = run_batch(clip, model)  # (8, 768)
                        feats = feats.squeeze()
                        clip_feat.append(feats)
                else:
                    clip_feat = np.zeros(shape=(num_clips, 8, 768))
                clip_feat = np.asarray(clip_feat)  # (16, 8, 768)
                ...
                if valid:
                    clip_feat = model(clip_torch)  # (16,512)
...
if __name__ == '__main__':
    ...
    # network params
    parser.add_argument('--model', default='Swin', choices=['resnet101', 'resnext101','ViViT', 'Swin', 'Clip'], type=str)
